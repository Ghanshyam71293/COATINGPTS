        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (string.IsNullOrEmpty(Request.QueryString["Type"]))
                {
                    ...
                    parameters.Clear();
                    parameters.Add("@UserId", Session["UserId"].ToString());
                    ds = clsm.senddataset_Parameter("select * from FinalGetNextBatch where Uname=@UserId  ", parameters);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                        {
            ...
            if (Convert.ToString(response1.StatusCode) == "OK")
            {
                ...
                if (!string.IsNullOrEmpty(V_NEXT_BATCH))
                {
                    ...
                    try
                    {
                        ...
                        cmd.CommandText = "FinalGetNextBatchSp";
                    ...
                    // Response.Redirect("NewBatchList.aspx");
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('SO/LineItem Updated successfully!');window.location ='FinalScan.aspx';", true);
                    return;
