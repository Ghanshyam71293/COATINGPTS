    ...
    public partial class TakePicture : System.Web.UI.Page
    {
        ...
        public static string Upload(string image)
        {
            ...
            try
            {
                ...
                string filePath = HttpContext.Current.Server.MapPath(string.Format("~/pts/INBOUND/Uploads/{0}.jpg", fileName));
                ...
                {
                    FileInfo F1 = new FileInfo(HttpContext.Current.Request.ServerVariables["Appl_Physical_Path"] + "pts\\INBOUND\\Uploads\\" + Convert.ToString(HttpContext.Current.Session["fileName"]));
                    if (F1.Exists)
                    {
                        F1.Delete();
        ...
        {
            DataTable dt = (DataTable)(HttpContext.Current.Session["NewINBOUNDBatch"]);
            DataRow dr = null;
            dr = dt.NewRow();
            dr["Codetype"] = "";
            dr["Code"] = customers.ToUpper().Trim();
            dt.Rows.Add(dr);
            HttpContext.Current.Session["NewINBOUNDBatch"] = dt;
        }
