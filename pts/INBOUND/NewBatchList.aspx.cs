...
namespace PTSCOATING.pts.INBOUND
{
    public partial class NewBatchList : System.Web.UI.Page
    {
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static List<string> GetAutoCompleteHeatNo(string query)
        {
            if (string.IsNullOrWhiteSpace(query))
            {
                return new List<string>();
            }
            DataTable dt = (DataTable)System.Web.HttpContext.Current.Session["DtHeatNo"];
            if (dt == null || !dt.Columns.Contains("HEATN"))
            {
                return new List<string>();
            }
            var suggestions = dt.AsEnumerable()
                .Where(row => !row.IsNull("HEATN") &&
                              row.Field<string>("HEATN").IndexOf(query, StringComparison.OrdinalIgnoreCase) >= 0)
                .Select(row => row.Field<string>("HEATN")).Distinct().Take(5)
                .ToList();
            return suggestions;
        }
        ...
        {          
            ...
            for (int i = 0; i < repDetails.Items.Count; i++)
            {
                ...
                HiddenField hiddenFbm = (HiddenField)repDetails.Items[i].FindControl("hiddenFbm") as HiddenField;
                HiddenField hiddenFbt = (HiddenField)repDetails.Items[i].FindControl("hiddenFbt") as HiddenField;

                txtbt.Text = hiddenFbt.Value;
        ...
        protected void repDetails_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
               ...
               // DropDownList ddlmtr = e.Item.FindControl("ddlmtr") as DropDownList;
                //if (ddlmtr != null)
                //{
                //    ddlmtr.SelectedIndexChanged += ddlmtr_SelectedIndexChanged;
                //}
                ...
                HiddenField hiddenFbm = e.Item.FindControl("hiddenFbm") as HiddenField;
                HiddenField hiddenbt = e.Item.FindControl("hiddenbt") as HiddenField;
                HiddenField hiddenFbt = e.Item.FindControl("hiddenFbt") as HiddenField;
                ...
                if (Session["DtCustPipeDetails"] != null && !string.IsNullOrEmpty(Session["DtCustPipeDetails"].ToString()))
                {
                    ...
                    txtcustpipeno.AutoPostBack = true;
                }
                else
                {
                   // GetCustPipeDetails(ddlmtr);
                    ...
                    txtcustpipeno.AutoPostBack = false;
                }
                ...
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('" + response1.Content.ToString() + "');window.location ='InboundScan.aspx';", true);
                    return;
                }
                if (Status1 != "S")
                {
                ...
                if (Session["DtCustPipeDetails"] is DataTable dt && dt.Rows.Count > 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(inputValue))
                    {
                        var results = dt.AsEnumerable()
                            .Where(row => !row.IsNull("CUSTP") && row.Field<string>("CUSTP").Equals(inputValue, StringComparison.OrdinalIgnoreCase))
                            .Select(row => new
                            {
        ...
        protected void ddlHeatNo_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (Session["DtCustPipeDetails"] is DataTable dt && dt.Rows.Count > 0)
            {
                DropDownList ddlHeatNo = (DropDownList)sender;
                RepeaterItem item = (RepeaterItem)ddlHeatNo.NamingContainer;
                TextBox txtheatdno = (TextBox)item.FindControl("txtheatdno");
                TextBox txtbm = (TextBox)item.FindControl("txtbm");
                TextBox txtcustpipeno = (TextBox)item.FindControl("txtcustpipeno");
                if (ddlHeatNo.SelectedValue != "NA")
                {
                    ...
                    txtheatdno.Text = ddlHeatNo.SelectedItem.Text; //ddlHeatNo.SelectedItem.Text;
                    txtbm.Enabled = false;
                }
                else
                {
