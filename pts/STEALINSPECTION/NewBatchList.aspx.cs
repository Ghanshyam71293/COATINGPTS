...
namespace PTSCOATING.pts.STEALINSPECTION
{
    public partial class NewBatchList : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (Session["NewStealInsBatch"] == null)
                {
                    Response.Redirect("StealInsScan.aspx");
                }
                DataSet ds = new DataSet();
                parameters.Clear();
                parameters.Add("@UserId", Session["UserId"].ToString());
                ds = clsm.senddataset_Parameter("select m.*,wcm.WorkCenter,wcm.WorkStationId from login m inner join WorkCenterRightMaster wcm on m.userid=wcm.Userid where m.UserId=@UserId and WorkCenter='STEAL INSPECTION' and m.Status=1 ", parameters);
        ...
        {
            DataTable dt = (DataTable)(Session["NewStealInsBatch"]);
            if (dt.Rows.Count > 1)
            {
        ...
        protected void SendMainForm()
        {
            ...
            if (StatusFinal == "S")
            {
                Session["NewStealInsBatch"] = null;
                repDetails.DataSource = null;
                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('" + MESSAGE + "');window.location ='StealInsScan.aspx';", true);
                return;
            }
            else
            {
        ...
        protected void ddlstatus_SelectedIndexChanged(object sender, EventArgs e)
        {
            for (int i = 0; i < repDetails.Items.Count; i++)
            {
                ...
                DropDownList ddlstatus = (DropDownList)repDetails.Items[i].FindControl("ddlstatus");
                parameters.Clear();
                clsm.Fillcombo_Parameter("select ReasonName,ReasonName as b from SteelStatusReason where status=1 and StatusCode='" + ddlstatus.SelectedValue + "' order by StatusCode asc", parameters, ddlreasons);
            }
        }
        protected void repDetails_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                parameters.Clear();
                clsm.Fillcombo_Parameter("select StatusName,StatusName as b from SteelstatusMaster where status=1 order by id asc", parameters, ddlstatus);
                ...
                if (statusCode1 == 0)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Server Not Respond!');window.location ='StealInsScan.aspx';", true);
                    return;
                }
                ...
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Something Worng- " + response1.Content.ToString() + "');window.location ='StealInsScan.aspx';", true);
                    return;
                }
                if (Status1 != "S")
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert(' " + Massage1 + "');window.location ='StealInsScan.aspx';", true);
                    return;
                }
